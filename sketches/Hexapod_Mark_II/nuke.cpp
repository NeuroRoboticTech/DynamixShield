...
/* IK Engine */
BioloidDynamixSerial *ikbioloid = NULL;
...
void doIK(){
    ...
    if(servo < maxs[RF_COXA-1] && servo > mins[RF_COXA-1])
        if(ikbioloid) ikbioloid->setNextPose(RF_COXA, servo);
    else{
    ...
    if(servo < maxs[RF_FEMUR-1] && servo > mins[RF_FEMUR-1])
        if(ikbioloid) ikbioloid->setNextPose(RF_FEMUR, servo);
    else{
    ...
    if(servo < maxs[RF_TIBIA-1] && servo > mins[RF_TIBIA-1])
        if(ikbioloid) ikbioloid->setNextPose(RF_TIBIA, servo);
    else{
    ...
    if(servo < maxs[RR_COXA-1] && servo > mins[RR_COXA-1])
        if(ikbioloid) ikbioloid->setNextPose(RR_COXA, servo);
    else{
    ...
    if(servo < maxs[RR_FEMUR-1] && servo > mins[RR_FEMUR-1])
        if(ikbioloid) ikbioloid->setNextPose(RR_FEMUR, servo);
    else{
    ...
    if(servo < maxs[RR_TIBIA-1] && servo > mins[RR_TIBIA-1])
        if(ikbioloid) ikbioloid->setNextPose(RR_TIBIA, servo);
    else{
    ...
    if(servo < maxs[LF_COXA-1] && servo > mins[LF_COXA-1])
        if(ikbioloid) ikbioloid->setNextPose(LF_COXA, servo);
    else{
    ...
    if(servo < maxs[LF_FEMUR-1] && servo > mins[LF_FEMUR-1])
        if(ikbioloid) ikbioloid->setNextPose(LF_FEMUR, servo);
    else{
    ...
    if(servo < maxs[LF_TIBIA-1] && servo > mins[LF_TIBIA-1])
        if(ikbioloid) ikbioloid->setNextPose(LF_TIBIA, servo);
    else{
    ...
    if(servo < maxs[LR_COXA-1] && servo > mins[LR_COXA-1])
        if(ikbioloid) ikbioloid->setNextPose(LR_COXA, servo);
    else{
    ...
    if(servo < maxs[LR_FEMUR-1] && servo > mins[LR_FEMUR-1])
        if(ikbioloid) ikbioloid->setNextPose(LR_FEMUR, servo);
    else{
    ...
    if(servo < maxs[LR_TIBIA-1] && servo > mins[LR_TIBIA-1])
        if(ikbioloid) ikbioloid->setNextPose(LR_TIBIA, servo);
    else{
    ...
    if(servo < maxs[RM_COXA-1] && servo > mins[RM_COXA-1])
        if(ikbioloid) ikbioloid->setNextPose(RM_COXA, servo);
    else{
    ...
    if(servo < maxs[RM_FEMUR-1] && servo > mins[RM_FEMUR-1])
        if(ikbioloid) ikbioloid->setNextPose(RM_FEMUR, servo);
    else{
    ...
    if(servo < maxs[RM_TIBIA-1] && servo > mins[RM_TIBIA-1])
        if(ikbioloid) ikbioloid->setNextPose(RM_TIBIA, servo);
    else{
    ...
    if(servo < maxs[LM_COXA-1] && servo > mins[LM_COXA-1])
        if(ikbioloid) ikbioloid->setNextPose(LM_COXA, servo);
    else{
    ...
    if(servo < maxs[LM_FEMUR-1] && servo > mins[LM_FEMUR-1])
        if(ikbioloid) ikbioloid->setNextPose(LM_FEMUR, servo);
    else{
    ...
    if(servo < maxs[LM_TIBIA-1] && servo > mins[LM_TIBIA-1])
        if(ikbioloid) ikbioloid->setNextPose(LM_TIBIA, servo);
    else{
