...
void setup() { 
  ...
  dbgTerminal.begin(57600); // Serial monitor
  espSerial.begin(115200); // ESP8266
    
  while(!dbgTerminal);
  dbgTerminal.println("Starting setup");
  ...
  //connect to router
  if(connectWiFi("TRENDnet812_2.4GHz_RPBS", "8124RF00337")) {
    isConnected = true;
    ...
    isConnected = false;
    dbgTerminal.println();
    dbgTerminal.println(F("Failed to connect."));    
  }
...
bool WaitForResponse(String success, String fail, int wait = 200)
{
  ...
  dbgTerminal.println("Waiting for Success: '" + 
  ...
  for(int i=0; i<wait; i++)
  {
    while(espSerial.available())
    {
      c1 = espSerial.read();
      results += c1;
      dbgTerminal.print(c1);  

      if(results.endsWith(success)) {
        dbgTerminal.println("");
        dbgTerminal.println("Found: " + success);
        return true;
      }

      if(results.endsWith(fail)) {
        dbgTerminal.println("");
        dbgTerminal.println("Found: " + fail);
        return false;
      }
    }
    delay(500);
  ...
  dbgTerminal.println("");
  dbgTerminal.println("Failed to find values.");
}
...
boolean connectWiFi(String NetworkSSID,String NetworkPASS) {
  ...
  return WaitForResponse("OK", "FAIL");
}
