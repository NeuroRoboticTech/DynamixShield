/*
  CommanderSS.cpp - Library for interfacing with ArbotiX CommanderSS
...
/* Constructor */
CommanderSS::CommanderSS(){
    index = -1;
    status = 0;
	stream = NULL;
}

CommanderSS::CommanderSS(SoftwareSerial *ss){
    index = -1;
    status = 0;
	stream = ss;
}

void CommanderSS::begin(unsigned long baud){
	if(stream != NULL)
		stream->begin(baud);
}

/* SouthPaw Support */
void CommanderSS::UseSouthPaw(){
    status |= 0x01;
}

/* process messages coming from CommanderSS 
 *  format = 0xFF RIGHT_H RIGHT_V LEFT_H LEFT_V BUTTONS EXT CHECKSUM */
int CommanderSS::ReadMsgs(){
	if(stream != NULL) {
